...
class StreamManager {
  ...
  void unregisterPeerConnection(String viewerId,
  ...
  Future<void> recoverWebRTC() async {
    final handlers =
        List<Future<void> Function()>.from(_recoveryHandlers.values);
  ...
  String? deviceToken;
  String server = CameraService.server;
  ...
  Future<void> loadIceServers() async {
    try {
      iceServers = await CameraService.fetchIceServers();
    } catch (_) {}
  }

  Future<void> startCamera() async {
    await stopMedia();
    localStream = await CameraService.getUserMedia(
      audio: true,
      video: true,
      videoConstraints: {
        "mandatory": {
  ...
  Future<void> startScreen() async {
    await stopMedia();
    localStream = await ScreenCaptureService.startScreenCapture();
    running = true;
  }
  ...
  Future<void> connectWebSocket() async {
    ...
    ws = await WebSocket.connect('$wsUrl/signal');
  ...
  // ✅ الإصلاح: backoff تصاعدي بدل تكرار سريع
  void _scheduleReconnect() {
    if (!running) return;
    _reconnectTimer?.cancel();
    final delay = Duration(
        seconds: math.min(30, 3 * (_reconnectAttempts + 1)));
    _reconnectAttempts++;
  ...
  Future<void> saveSource(String source) async {
    ...
    await prefs.setString("last_stream_source", source);
  }

  Future<String> getSavedSource() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getString("last_stream_source") ?? "camera";
  }
}
